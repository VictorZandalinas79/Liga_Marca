...
import { computeOutOfOrderLocks, isLockActive, DEFAULT_LOCK_OFFSETS, type FixtureLite } from './src/lib/locked-teams-core'
...
async function test() {
  ...
  if (!fixtures) return
  const locks = computeOutOfOrderLocks(fixtures as unknown as FixtureLite[], DEFAULT_LOCK_OFFSETS)
