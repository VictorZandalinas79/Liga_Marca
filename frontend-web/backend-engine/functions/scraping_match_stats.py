...
class MatchStatsScraper(BaseScraper):
    """
    Scraper de estadísticas detalladas del partido (Posesión, Tiros, etc.).
    Usa el endpoint 'matchstats' con la estructura limpia.
    """
    ...
    def download_stats_for_season(self, competition_name, season_name, df_matches):
        """
        Descarga stats para cada partido en df_matches.
        """
        folder = f"{competition_name}/{season_name}/match_stats"
        
        print(f"📊 Iniciando descarga de Match Stats para: {competition_name} - {season_name}")
        ...
        for index, row in df_matches.iterrows():
            ...
            # --- CONSTRUCCIÓN URL ---
            # Endpoint: matchstats
            # Estructura: /matchstats/{KEY}/{MATCH_ID}
            # Parámetros: sps=widgets (Vital), detailed=yes (para todas las stats)
            url = (
                f"https://api.performfeeds.com/soccerdata/matchstats/"
                f"{self.sdapi_outlet_key}/"
                f"{match_id}"
                f"?_fmt=jsonp&_rt=c&_lcl=en&sps=widgets&_clbk=callback&detailed=yes"
            )
            ...
            try:
                ...
                if start_idx != -1 and end_idx != -1:
                    ...
                    # A veces no hay stats detalladas para partidos viejos o futuros
                    # print(f"   ⚠️ Sin stats detalladas para {match_id}")
                    count_err += 1
        ...
        print(f"\n🏁 Fin de descarga de Match Stats.")
        print(f"   ✅ Descargados: {count_ok}")
        print(f"   ⏭️ Saltados: {count_skip}")
        print(f"   ❌ Errores/Vacíos: {count_err}")