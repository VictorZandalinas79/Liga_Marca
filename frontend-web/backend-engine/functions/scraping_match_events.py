...
class MatchEventScraper(BaseScraper):
    """
    Scraper de eventos (Goles, Tarjetas, Cambios).
    Usa el endpoint 'matchevent' descubierto.
    """
    ...
    def download_events_for_season(self, competition_name, season_name, df_matches):
        """
        Descarga los eventos para cada partido en df_matches.
        """
        folder = f"{competition_name}/{season_name}/events"
        
        print(f"⚡ Iniciando descarga de eventos (matchevent) para: {competition_name} - {season_name}")
        ...
        for index, row in df_matches.iterrows():
            ...
            # --- CONSTRUCCIÓN URL CORREGIDA ---
            # Endpoint: matchevent
            # Estructura: /matchevent/{KEY}/{MATCH_ID}
            # Parametros: _rt=c, _lcl=en, sps=widgets
            url = (
            ...
            try:
                ...
                if start_idx != -1 and end_idx != -1:
                    ...
                    # A veces matchevent devuelve vacío si no hay datos, pero no es error 403
