...
async function run() {
  const { data: fixtures } = await supabase.from('fixtures').select('id, matchday, start_time').eq('matchday', 1).order('start_time', { ascending: false }).limit(5);
  console.log('Last fixtures MD 1:', fixtures);
}
run();
